...
export type AuthenticatedUser = Omit<User, 'role'> & { role: UserRole };

function normalizeRole(role: string): UserRole {
  return isUserRole(role) ? role : 'worker';
}

function normalizeUser(user: User | null): AuthenticatedUser | null {
  if (!user) return null;
