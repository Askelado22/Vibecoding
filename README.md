...
- **Рабочая** — двухколоночный рабочий экран (карточка товара + сайдбар) с фиксированным нижним тулбаром. Здесь живут PathBuilder, подсказки, статусный сегментированный контрол, тосты и аккордеон комментария.
- **Общий список** — компактная зебра-таблица A..M с фильтрами без горизонтального скролла.
- **Админ-панель** — карточки метрик, блоки синхронизации, таблицы подсказок и пользователей.

Тема и отступы: все цвета и размеры берутся из `frontend/static/frontend/theme.css` (CSS Custom Properties). Активная вкладка — pill-кнопка с маркером-точкой, нижний тулбар состоит из трёх зон (навигация, быстрое поле, CTA) и прогресс-полосы. Комментарий редактируется в выдвижном drawer-е, который запоминает состояние между сессиями.

### Manual UI чеклист

1. **Tabs & header** — активная вкладка отображается голубым pill, текст не перекрывается, хедер прилипает при скролле.
2. **Product media** — изображение всегда внутри синей рамки (object-fit: contain), миниатюры не прыгают.
3. **PathBuilder** — двойной пробел превращается в `>`, чипы можно перетаскивать и удалять, валидация выделяет неверные сегменты.
4. **Bottom toolbar** — на ширинах 360/768/1024/1440px кнопки не наезжают, тулбар не перекрывает контент, прогресс обновляется.
5. **Комментарии** — drawer сворачивается/разворачивается, счётчик символов обновляется, выбранные шаблоны добавляются.
6. **Состояние завершения** — кнопка «Завершить карточку» активируется только при валидных крошках + статусе, подсказка подсказывает причину блокировки.
7. **Списки** — таблицы не имеют горизонтального скролла, колонка L read-only, при наведении строка подсвечивается.
8. **Тосты** — отображаются в правом верхнем углу, исчезают через ~3 секунды, имеют корректный цвет бордюра (синий/зелёный/розовый).

Все запросы выполняются поверх публичного API, поэтому можно работать и из UI, и напрямую.
