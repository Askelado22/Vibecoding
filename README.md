...
## Чек-лист ручной проверки UI

Перед сдачей изменений по интерфейсу убедитесь, что выполнены шаги:

1. Рабочая вкладка на 1920×1080 показывает PathBuilder, подсказки, статус и описание (≥560px) без вертикальной прокрутки страницы.
2. Заголовки вкладок выглядят как pill-кнопки, маркер точки не перекрывает текст.
3. Изображение товара сохраняет пропорции 16:9 и не выходит за рамки на ширинах 360/768/1024/1440.
4. PathBuilder позволяет добавлять сегменты двойным пробелом, перетаскивать и удалять чипы; валидации подсвечивают ошибки.
5. Комментарий открывается/закрывается из нижней панели и запоминает состояние между визитами.
6. Нижний Toolbar не перекрывает контент, на узких экранах складывается в две строки, кнопки не накладываются.
7. Таблицы в «Общем списке» и «Админ-панели» имеют липкий заголовок, колонка L визуально read-only.
8. Цвета соответствуют токенам темы: без градиентов, контраст текста ≥ 4.5:1.
9. Тосты появляются в правом верхнем углу и скрываются кликом или автоматически.
